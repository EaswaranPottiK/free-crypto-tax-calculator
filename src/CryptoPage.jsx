...
const CryptoPage = () => {
  ...
  function reducer(state, action){
    ...
    if(investmentType){
      if((Number(itemToReturn.salePrice) - Number(itemToReturn.purchasePrice)- Number(itemToReturn.expenses))>0){
        itemToReturn.greenbox = (Number(itemToReturn.salePrice) - Number(itemToReturn.purchasePrice)- Number(itemToReturn.expenses))*0.5
      }
      else {
        itemToReturn.greenbox = Number(itemToReturn.salePrice) - Number(itemToReturn.purchasePrice)- Number(itemToReturn.expenses)
      }
    }
    else {
      itemToReturn.greenbox = Number(itemToReturn.salePrice) - Number(itemToReturn.purchasePrice)- Number(itemToReturn.expenses)
    ...
    if (itemToReturn.annualIncome <=18200){
      ...
      itemToReturn.bluebox = Number(itemToReturn.greenbox)*0.19
    }
    else if(itemToReturn.annualIncome <=120000){
      itemToReturn.taxRate = '$5,092 + 32.5% of excess over $45,000'
      itemToReturn.bluebox = Number(itemToReturn.greenbox)*0.325 + 5092
    }
    else if(itemToReturn.annualIncome <=180000){
      itemToReturn.taxRate = '$29,467 + 37% of excess over $120,000'
      itemToReturn.bluebox = Number(itemToReturn.greenbox)*0.37 + 29467
    }
    else{
      itemToReturn.taxRate = '$51,667 + 45% of excess over $180,000'
      itemToReturn.bluebox = Number(itemToReturn.greenbox)*0.45 + 51667
    }
    return itemToReturn
  }

  const [state,dispatch] = useReducer(reducer, {greenbox:0,bluebox:0, taxRate:'0%',purchasePrice:0,salePrice:0,expenses:0,annualIncome:18200,capitalGainAmount:0,discountForLongTermGains:0})
      ...
      <div>
        ...
        <div className='flex justify-around md:flex-row flex-col '>
          ...
          <div className='flex flex-col text-sm'>
            ...
            <div className='flex md:w-52 text-lg w-[100%]'>
              ...
              <input type='number' onChange={(e)=>dispatch({purchasePrice:e.target.value})}  className='w-[100%] md:w-48 min-h-12 outline-none bg-[#edeeef] rounded-r-lg'/>
              {/* onChange={(e)=>{}} */}
            </div>
          </div>
          ...
          <div className='flex flex-col text-sm'>
            ...
            <div className='flex  md:w-52 text-lg w-[100%]'>
              <input type='text' value=' $' className='w-4 min-h-12  bg-[#edeeef] rounded-l-lg' readOnly/>
              <input type='number'  onChange={(e)=>dispatch({salePrice:e.target.value})}  className='w-[100%] md:w-48 min-h-12 outline-none bg-[#edeeef] rounded-r-lg'/>
            </div>
          </div>
        ...
        <div className='flex justify-around flex-col md:flex-row'>
          <div className='flex flex-col text-sm'>
            ...
            <div className='flex w-[100%] md:w-52 text-lg'>
              <input type='text' value=' $' className='w-4 min-h-12  bg-[#edeeef] rounded-l-lg' readOnly/>
              <input type='number'  onChange={(e)=>dispatch({expenses:e.target.value})}  className='w-[100%] md:w-48 min-h-12 outline-none bg-[#edeeef] rounded-r-lg'/>
            </div>
          </div>
        ...
        <div className='flex justify-around flex-col md:flex-row'>
          <div className='flex flex-col text-sm'>
            ...
            <div>
              <select  onChange={(e)=>dispatch({annualIncome:e.target.value})}  className='flex w-[100%] md:w-52 text-lg bg-[#edeeef] h-12 rounded-md'>
